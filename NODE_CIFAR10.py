...
func = ConvODEF(64)
ode = NeuralODE(func)
model = ContinuousNeuralCIFAR10Classifier(ode)
if use_cuda:
    model = model.cuda()
#%%
img_std = 0.3081
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        with torch.no_grad():
            for batch_idx, (data, target) in tqdm(enumerate(test_loader_real),
                ...
                validation_iters += 1
            print("Test loss: {:.5f}%".format(np.mean(validation_epoch_losses))
                  )
            print("Test Accuracy: {:.3f}%".format((validation_epoch_accuracy\
                                                   * 100)/num_items_validation)
                  )
    retrain_model = False

#%%
torch.save(model.state_dict(), "CNNODE5.pth")
...
plt.plot(train_accuracy,label="Accuraccy")
plt.xlabel("iterations")
plt.ylabel("Accuracy")
plt.legend()
plt.show()
